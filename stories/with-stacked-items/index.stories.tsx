...
export default {
  ...
  title: 'react-easy-sort/Stacked Items',
  parameters: {
...
export const Demo: Story<StoryProps> = ({ count, hasStackedItems }: StoryProps) => {
  ...
  return (
    <SortableList
      ...
      hasStackedItems={hasStackedItems}
    >
      {items.slice(0, count).map(({ name, image }, index) => (
        <SortableItem key={name}>
          <div style={{ left: index * 44, top: index * 44 }} className={classes.item}>
            <Avatar
